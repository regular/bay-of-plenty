...
test('eval', t=>{
  ...
  const bop = spawn_bop([
    ...
    ;(async function() {
      const appTarget = await browser.waitForTarget(t=>t.url().includes('eval.js'))
  ...
  bop.stderr.on('data', data =>{
    process.stdout.write(data)
    if (data.includes('Refused to evaluate a string as JavaScript')) {
      t.pass('Javascript was not evaluated (which is good)')
      clickClose()
    }
  })
