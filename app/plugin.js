...
exports.init = function (ssb, config) {
  ...
  ssb.ws.use(function (req, res, next) {
    ...
    if (u.pathname.startsWith('/launch/')) {
      const filepath = decodeURIComponent(u.pathname.split('/')[2])
      const bootKey = decodeURIComponent(u.pathname.split('/')[3])
      debug('request for launch page %s, bootKey: %s', filepath, bootKey)
      res.statusCode = 200
      res.setHeader('Content-Type', 'text/html')
      return sendLaunchPage(filepath, res)
    }
...
function sendLaunchPage(filepath, res) {
  const body = BufferList()
  body.append("<!-- generated by Bay of Plenty's launch plugin -->\n\n")
  const hs = hyperstream({body})

  const browserify = Browserify()
  browserify.transform(require('brfs'))
  browserify.add(join(__dirname, filepath))
  browserify.bundle()
